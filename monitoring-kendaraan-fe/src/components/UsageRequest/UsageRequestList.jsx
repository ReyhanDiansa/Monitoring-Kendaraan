...
const UsageRequestList = () => {
  ...
  const fetchUsageRequests = useCallback(
    async (page) => {
      try {
        let response;
        if (searchQuery.trim() === "") {
          response = await api.get(`/api/usage-request?page=${page}`);
  ...
  const searchUsageRequests = async (keyword, page) => {
    try {
      const response = await api.post(`/api/usage-request/filter/${keyword}`, {
        page,
      });
      setUsageRequests(response.data.data.data);
          setTotalPages(response.data.data.last_page);
  ...
  const exportData = async () => {
    ...
    let url = '/api/usage-request/get-data/for-export';
    if (keyword !== '') {
      url += `?keyword=${keyword}`;
    }
    try {
                ...
                {usageRequests?.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="border px-4 py-2 flex flex-col gap-2 items-center justify-center">
                      ...
                      <div>
                        <button
                          className="bg-yellow-500 text-white hover:text-yellow-500 hover:bg-white hover:ring-yellow-600 hover:ring-2 focus:ring-4 focus:ring-yellow-200 text-sm px-2 py-2 rounded mr-2"
                          onClick={() => handleDetail(item.id)}
                        >
                          Detail
                        </button>
                        <button
                          className="bg-red-500 text-white hover:text-red-500 hover:bg-white hover:ring-red-700 hover:ring-2 focus:ring-4 focus:ring-red-200 text-sm px-2 py-2 rounded"
